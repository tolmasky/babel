...
import { toParsingErrorClasses } from "./error";
import type { TokenType } from "../tokenizer/types";
import { tokenLabelName } from "../tokenizer/types";

export default toParsingErrorClasses({
  ...
  InvalidOrUnexpectedToken: ({ found } : { found: string }) => `Unexpected character '${found}'.`,
  InvalidParenthesizedAssignment: () => "Invalid parenthesized assignment pattern.",
  InvalidPrivateFieldResolution: ({ name }: { name: string }) => `Private name #${name} is not defined.`,
  ...
  VarRedeclaration: ({ name }: { name: string }) => `Identifier '${name}' has already been declared.`,
